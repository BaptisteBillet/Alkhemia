...
public class Potion_vent : Potion {
...
//    private int gachette_actif;
    ...
    //Spécial potion de vent
    private float max_speed_player;
    public float boost_speed_sort;
    ...
    void initialisation_bullet()
    {
        ...
        bullet_script.name = "bullet_vent";                  //La potion envoie des bullets avec ce nom
	...
	void Update () 
    {
        ...
        if (((Input.GetMouseButtonDown(1) || gachette_sort == true) && quantite - sort_depense >= 0) && player_potion_script.potion_actuel == numero )
        {
           ...
           if(active==false)
           {
            ...
            //On change de layer pour voler
            MoveToLayer(this.gameObject.transform.parent, LayerMask.NameToLayer("Vol"));
    ...
    IEnumerator fin_sort()
    {
        while(refresh>=0)
        {
            player_script.max_moveSpeed += boost_speed_sort;
            refresh--;
            yield return new WaitForSeconds(1f);
        }
        player_script.max_moveSpeed = max_speed_player;
        //On revient sur le sol
        MoveToLayer(this.gameObject.transform.parent, LayerMask.NameToLayer("Default"));
